...
@st.cache_resource
def load_xgb():
    pkl_path = PROJECT_ROOT / "models" / "xgb_model.pkl"
    json_path = PROJECT_ROOT / "models" / "xgb_model.json"
    if not pkl_path.exists() and not json_path.exists():
        st.error(f"No model found at {pkl_path} or {json_path}")
        raise FileNotFoundError("Missing models/xgb_model.pkl or models/xgb_model.json")
